...
function _M.new(self, key, salt, _cipher, _hash, hash_rounds)
    ...
    return setmetatable({
      ...
      _block_size = C.EVP_CIPHER_CTX_block_size(encrypt_ctx)
      }, mt)
end

function _M.encrypt(self, s)
    local s_len = #s
    local data_len = math.floor(((s_len - 1) / self._block_size) + 1) * self._block_size
    local buf = ffi_new("unsigned char[?]", data_len)
    local out_len = ffi_new("int[1]")
    local ctx = self._encrypt_ctx
    local data_char = ffi.new("unsigned char[?]", data_len)
    ffi.fill(data_char, data_len, 0)
    ffi.copy(data_char, s, s_len)
    ...
    if C.EVP_EncryptUpdate(ctx, buf, out_len, ffi_str(data_char, data_len), data_len) == 0 then
        return nil
    end
...
function _M.decrypt(self, s)
    ...
    while buf[s_len - padding_size - 1] == 0 do
